...
class MovieState(TypedDict):
    ...
    recommended_movie: str  # Title of the recommended movie
...
def create_graph():
    ...
    workflow.add_node("fetch_recommended_metadata", lambda state: metadata_agent.fetch_metadata({"input": state.get("recommended_movie", "Unknown")}))
    ...
    workflow.add_edge("fetch_metadata", "llm_recommendation")
    workflow.add_edge("llm_recommendation", "fetch_recommended_metadata")
    workflow.add_edge("fetch_recommended_metadata", "analyze_sentiment")
    workflow.add_edge("analyze_sentiment", "fetch_box_office")
    workflow.add_edge("fetch_box_office", END)
